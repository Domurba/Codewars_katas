#2021-11-22 18:51:04.725000+00:00
...
def seq(n):
    return all((str(int(v) + 1) == n[i + 1] or int(v) + 1 == 10 and n[i + 1] == '0' for i, v in enumerate(n[:-1])))


def rev(n):
    return all((str(int(v) - 1) == n[i + 1] for i, v in enumerate(n[:-1])))


def is_interesting(number, awesome_phrases):
    if number > 97:
        if number in [98,99]: return 1
        num, nums = str(number), [number + 1, number + 2]
        if (
            number in awesome_phrases
            or num == num[::-1]
            or re.match(r"^\d0+$", num)
            or len(set(num)) == 1
            or seq(num)
            or rev(num)
        ):
            ...
            any(x in awesome_phrases for x in nums)
            or any(str(x) == str(x)[::-1] for x in nums)
            or re.match(r"^\d0+$", str(nums[1]))
            or re.match(r"^\d0+$", str(nums[0]))
            or len(set(str(nums[1]))) == 1
            or seq(str(nums[0]))
            or seq(str(nums[1]))
            or rev(str(nums[0]))
            or rev(str(nums[1]))
